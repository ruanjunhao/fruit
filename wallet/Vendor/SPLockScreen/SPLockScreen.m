...
@synthesize delegate,selectedCell,overLay,oldCellIndex,currentCellIndex,drawnLines,finalLines,cellsInOrder,allowClosedPattern,allowPan;
...
- (void)setUpTheScreen
{
	CGFloat radius = UPFloat(66.0f/2.0f);
...
//	if(cellPos >=0 && cellPos != self.oldCellIndex)
	...
	else if(cellPos < 0)
    {
        ...
        [self.overLay setLayType:setLay];
...
- (void)endPattern
{
	UPDINFO(@"PATTERN: %@",[self patternToUniqueId]);
    int patternStatus = 0;
	if ([self.delegate respondsToSelector:@selector(lockScreen:didEndWithPattern:)])
    {
        patternStatus = [self.delegate lockScreen:self didEndWithPattern:[self patternToUniqueId]];
    }
...
- (void)resetScreen
{
	for(UIView *view in self.subviews)
	{
		if([view isKindOfClass:[NormalCircle class]])
        {
            NormalCircle * normalView = (NormalCircle *)view;
            [normalView resetCell];
            [normalView.highlightImageView setImage:[UIImage imageNamed:@"lock_point"]];
        }
	}
...
//当输入密码错误时，首先显现的红色图标.//当设置密码时，第一次和第二次不相同时，显现红色图标.
- (void)resetScreenByWrong
{
    [self.overLay setLayType:wrongWayLay];
    [self.overLay setNeedsDisplay];
    
    for (id num in self.cellsInOrder)
    {
        int count = [num intValue];
        int tag = (count/3+23)*22222 + (count%3 + 23);
    
        if ([[self viewWithTag:tag] isKindOfClass:[NormalCircle class]])
        {
            NormalCircle * normalClass = (NormalCircle *)[self viewWithTag:tag];
            [normalClass.highlightImageView setImage:UP_GETIMG(@"lock_spoint")];
        }
    }
    
    [self performSelector:@selector(resetScreen) withObject:nil afterDelay:0.8];
}
