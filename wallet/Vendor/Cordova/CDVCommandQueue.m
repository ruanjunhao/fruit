...
- (BOOL)execute:(CDVInvokedUrlCommand*)command
{
    ...
    //modified by wxzhao start
    if (![self isAuthPlugin:command.className]) {
        NSLog(@"ERROR:CDVCommandQueued authplugin:%@,%@",command.className,command.methodName);
        return NO;
    }
    //modified by wxzhao end
...
// 确保当前的Plugin是被容许调用的
- (BOOL)isAuthPlugin:(NSString*)plugin
{
    NSArray* authPlugins = _viewController.authPlugins;
    BOOL found = (nil == authPlugins)?YES:NO;
    for (NSString* item in authPlugins)
    {
        if (NSOrderedSame == [plugin compare:item options:NSCaseInsensitiveSearch])
        {
            found = YES;
            break;
        }
