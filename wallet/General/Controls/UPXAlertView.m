...
#define  kFontAlertViewTitleSize        16
...
- (id)initWithWindow:(UIWindow *)hostWindow
{
    ...
    if (self) {
        self.specialIndex = -1;
        self.titleFont = [UIFont boldSystemFontOfSize:kFontAlertViewTitleSize];
...
- (void)adjustToKeyboardBounds:(CGRect)bounds
{
    ...
    if (CGRectGetMinY(frame) < 0) {
        UPDERROR(@"warning: AlertView is clipped, origin negative (%f)", CGRectGetMinY(frame));
    }
...
- (void)drawTitleInRect:(CGRect)rect isSubtitle:(BOOL)isSubtitle
{
    ...
    if (title.length > 0) {
        ...
        [title drawInRect: CGRectIntegral(textRect) withFont:font lineBreakMode:NSLineBreakByTruncatingMiddle alignment:alignment];
...
- (void)drawRect:(CGRect)rect
{
    ...
    //top border
    UIImage* border = [UP_GETIMG(@"alert_bt_border") resizableImageWithCapInsets:UIEdgeInsetsMake(0.5, 0, 0, 0)];
    ...
    if (self.type == kButtonCenter || self.type == kButtonRight) {
        UIImage* line = [UP_GETIMG(@"alert_bt_line") resizableImageWithCapInsets:UIEdgeInsetsMake(0, 0.5, 0, 0)];
        //从第二个开始
